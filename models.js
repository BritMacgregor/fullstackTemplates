...
const sortAnswers = function(a, b) {
  ...
  //0 returns no change
  //+ (postiive) if b appears before a
...
//if the votes match order them by the last time they were updated,
  ...
  const AnswerSchema = new Schema();
      ...
      //We want to set this to the current date and time when every answer is created. //Mongoose can take care of that using an object to specify the default value.
      ...
      //When the answer is updated, we want to apply the updates to unanswered document. //We also want the answeres updated property to be the current time.
      ...
      AnswerSchema.methods.method("update", function(updates, callback) {
      ...
      //since we've already validated the string that is passed into the vote parameter, there are only 2 possiblities.
        AnswerSchema.methods.method("vote", function(vote, callback) {
  ...
  const QuestionSchema = new Schema({
    ...
    text: String,
    //as well as a time for creation
    //We want to set this to the current date and time when every question is created. //Mongoose can take care of that using an object to specify the default value.
